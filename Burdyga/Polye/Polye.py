...
{"word": "ВДОХНОВЕНИЕ", "hint": "Особое состояние, прилив творческих сил"},
    ...
    def show_treasure_box(self):
        ...
        if not current_player["is_human"]:
            ...
            # ИИ автоматически получает случайный бонус
            bonus_options = [
            ...
            # Проверяем, полностью ли отгадано слово после открытия буквы
            if "_" not in self.hidden_word:
                self.word_solved()
                return
        ...
        # Для человека-игрока показываем диалог
        dialog = tk.Toplevel(self.root)
        ...
        def choose_double_points():
            ...
            bonus = int(current_points * 0.5)  # 50% бонус
    ...
    def guess_letter(self, letter):
        ...
        if letter in self.current_word:
            ...
            # Увеличиваем счетчик правильных букв подряд
            current_player["consecutive_correct"] += 1
            
            # Проверяем, достигли ли 3 правильных букв подряд
            if current_player["consecutive_correct"] >= 3:
                current_player["consecutive_correct"] = 0
                self.update_player_indicators()
                
                # Показываем шкатулку после обновления отображения
            ...
            # Сбрасываем счетчик правильных букв
            current_player["consecutive_correct"] = 0
            self.update_player_indicators()
    ...
    def next_round(self):
        ...
        if self.game_round < max_rounds:
            ...
            # Сбрасываем множители и счетчики подряд угаданных букв
